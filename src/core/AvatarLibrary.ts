...
export class AvatarLibrary {
    ...
    /**
     * Save a new avatar to the library (prevents duplicates)
     */
    static async saveAvatar(name: string, imageData: string): Promise<SavedAvatar> {
        ...
        const existing = await this.getAvatars();
        const duplicate = existing.find(a => a.imageData === imageData);
        if (duplicate) {
            // Already exists, return the existing one instead of creating a copy
            return duplicate;
        }
        ...
        try {
            ...
            localStorage.setItem(AVATARS_KEY, JSON.stringify(customAvatars));
        } catch (e) {
