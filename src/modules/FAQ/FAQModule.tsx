...
export const FAQConfigComponent: React.FC<FAQConfigComponentProps> = ({
  ...
  const handleAddQuestion = () => {
    ...
    // Enforce FAQ limits
    if (subscription && subscription.limits && subscription.limits.faqs !== -1) {
      if (questions.length >= subscription.limits.faqs) {
        alert(`You've reached the limit of ${subscription.limits.faqs} FAQs for your ${subscription.tier} plan. Please upgrade to add more!`);
        return;
      }
    }
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex items-center justify-between">
        ...
        <div className={`text-xs font-semibold px-2 py-1 rounded-full ${questions.length >= (subscription?.limits?.faqs || 10) ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`}>
          {questions.length} / {subscription?.limits?.faqs === -1 ? '∞' : subscription?.limits?.faqs || 10} used
        </div>
      </div>
