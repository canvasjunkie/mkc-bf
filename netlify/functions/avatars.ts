...
export const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
    ...
    try {
        // Try to get the store - may fail if Blobs not configured
        let store;
        try {
            store = getStore(STORE_NAME);
        } catch (storeError) {
            console.warn('Netlify Blobs not available:', storeError);
            // Return empty array if blobs not configured
            ...
            return {
                statusCode: 503,
                headers,
                body: JSON.stringify({ error: 'Avatar storage not available' })
            };
        }
