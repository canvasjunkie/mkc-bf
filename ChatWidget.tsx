...
// NOTE: GeminiService removed - ALL Gemini calls go through backend /api/chat
...
// Strong default system prompt for all bots (especially free tier)
const DEFAULT_SYSTEM_PROMPT = `You are a friendly, professional AI assistant. Your role is to help website visitors by answering their questions clearly and concisely. Be helpful, polite, and guide users toward the information or services they need. If you don't know something, be honest about it and suggest they contact the business directly. Keep responses brief but informative.`;

/**
 * Generate or retrieve a unique visitor token for session isolation.
 * Each browser session gets its own token, keeping conversations private.
 * This prevents data leakage between different visitors on the same bot.
 */
const getVisitorToken = (botId: string): string => {
  const storageKey = `mk_visitor_${botId}`;

  // Check sessionStorage first (persists within tab session)
  let token = sessionStorage.getItem(storageKey);

  if (!token) {
    // Generate a unique token using crypto API (or fallback)
    if (typeof crypto !== 'undefined' && crypto.randomUUID) {
      token = crypto.randomUUID();
    } else {
      // Fallback for older browsers
      token = 'v_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 15);
    }
...
export const ChatWidget: React.FC<ChatWidgetProps> = ({ bot, preview = false }) => {
  // Generate unique visitor token for session isolation
  const visitorToken = useMemo(() => getVisitorToken(bot.id), [bot.id]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Log usage if in preview mode (within the Factory)
      if (preview) {
        const token = SubscriptionService.getToken();
        if (token) {
          await SubscriptionService.logMessageUsage(token);
        }
      }
  ...
  const processMessage = async (message: string): Promise<string> => {
    ...
    const provider = bot.settings.aiProvider || 'gemini';

    // RULE: OpenRouter is PRO ONLY - only if user has their own API key
    const hasBotOpenRouterKey = !!bot.settings.openRouterApiKey?.trim();

    // RULE: Gemini calls ALWAYS go through backend - NO direct frontend calls
    // This keeps the API key secure on the server
    if (provider === 'gemini') {
      // Always use server-side proxy for Gemini
      try {
        const response = await callChatProxy(message, 'gemini');
        await logAiInteraction('gemini', message, response);
        return response;
      } catch (error) {
        console.error('Gemini error:', error);
        return `I'm having trouble connecting. ${(error as any).message}`;
      }
    }

    // RULE: OpenRouter is ONLY for Pro users with their own API key
    if (provider === 'openrouter' && hasBotOpenRouterKey) {
      try {
        const response = await callOpenRouter(message);
        await logAiInteraction('openrouter', message, response);
        return response;
      } catch (error) {
    ...
    // Fallback: If OpenRouter selected but no key, use Gemini via backend
    try {
      const response = await callChatProxy(message, 'gemini');
      await logAiInteraction('gemini', message, response);
      return response;
  ...
  const logAiInteraction = async (provider: string, userMessage: string, aiResponse: string) => {
    try {
      await MemorySystem.logExperience(bot.id, {
        type: 'ai_interaction',
        provider,
  ...
  // NOTE: callGeminiWithKey REMOVED - All Gemini calls go through backend /api/chat
  ...
  const callOpenRouter = async (message: string): Promise<string> => {
    const messages = [
      {
        role: 'system',
        content: bot.settings.systemPrompt || DEFAULT_SYSTEM_PROMPT
      },
      ...conversationMemory,
      {
        role: 'user',
    ...
    // Only log in development to prevent sensitive data leakage
    ...
    const data = await response.json();
    // Never log full response data as it may contain personal info
    if (import.meta.env.DEV) {
  ...
  return (
    <div style={{
      ...
      {/* Chat Bubble - Shows bot avatar */}
      <div className={getBubblePosition()}>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-white font-bold transition-all duration-200 hover:scale-110 text-xl overflow-hidden"
        ...
        >
          {(() => {
            ...
            if (!avatarConfig || avatarConfig.type === 'emoji') {
              return <span className="text-2xl">{avatarConfig?.value || '🤖'}</span>;
            }
            return (
              <img
